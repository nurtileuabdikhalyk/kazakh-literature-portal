  ...
  <div class="alf-page">
    ...
    <div class="alf-header">
      ...
      <div>
        ...
        <p class="alf-sub">
          Деректер <strong>Supabase</strong>-ке сақталады ·
      ...
      <div class="preview-col">
        <div class="sticky-wrap">
          ...
          <div class="ls-info-card">
            <div class="li-head">
              <i class="pi pi-database" style="color:#3ecf8e"/> Supabase · lessons
            </div>
            <div class="li-rows">
              <div class="li-row">
                <span>Барлық сабақ</span>
  ...
  if (editId.value) {
    ...
    if (lesson) {
      Object.assign(form, {
        ...
        level: lesson.level || 'Орташа',
        author: lesson.author || '',
        duration: lesson.duration || '',
        description: lesson.description || '',
        fileUrl: lesson.fileUrl || '',
        cover: lesson.cover || '',
        classes: lesson.classes || ['8А'],
      })
    }
  } else {
...
async function save() {
  ...
  try {
    ...
    if (editMode.value && editId.value) {
      await updateLesson(editId.value, data)
      showToast('Сабақ Supabase-те жаңартылды!')
    } else {
      await addLesson(data)
      showToast('Сабақ Supabase-ке қосылды!')
      resetForm()
    }
  } catch (e) {
...
.type-btn.active i, .type-btn.active span {
  color: #fff;
}

.type-btn.video.active {
...
.preview-card.video::before {
  background: var(--rust);
}

.preview-card.text::before {
  background: var(--sage);
}

.preview-card.pdf::before {
  background: var(--navy);
}
...
.pc-badge.video {
  background: rgba(139, 58, 30, .9);
}

.pc-badge.text {
  background: rgba(58, 92, 58, .9);
}

.pc-badge.pdf {
  background: rgba(42, 58, 92, .9);
}
