...
function validate() {
  ...
  let ok = true
  if (!form.topic) {
    ve.topic = 'Тақырып таңдаңыз';
    ok = false
  }

  if (form.type === 'mcq') {
    if (!form.text.trim()) {
      ve.text = 'Сұрақ мәтінін жазыңыз';
      ok = false
    }
    if (!form.answer) {
      ve.answer = 'Дұрыс жауапты белгілеңіз';
      ok = false
    }
    if (mcqOptions.value.filter(o => o.trim()).length < 2) {
      ve.answer = 'Кем дегенде 2 нұсқа толтырыңыз';
      ok = false
    }
  } else if (form.type === 'truefalse') {
    if (!form.text.trim()) {
      ve.text = 'Тұжырым жазыңыз';
      ok = false
    }
    if (form.answer === null) {
      ve.answer = 'Дұрыс/Бұрыс таңдаңыз';
      ok = false
    }
  } else if (form.type === 'fillblank') {
    if (!form.text.trim()) {
      ve.text = 'Мәтінді жазыңыз';
      ok = false
    }
    if (!form.answersRaw.trim()) {
      ve.answers = 'Дұрыс жауаптарды жазыңыз';
      ok = false
    }
    if (blankCount.value === 0) {
      ve.text = 'Мәтінде ___ белгісі жоқ';
      ok = false
    }
  } else if (form.type === 'match') {
    if (!form.title.trim()) {
      ve.title = 'Атауын жазыңыз';
      ok = false
    }
    if (form.pairs.filter(p => p.left.trim() && p.right.trim()).length < 2) {
      ve.pairs = 'Кем дегенде 2 толық жұп керек';
      ok = false
    }
  }
  return ok
...
function removePair(idx) {
  if (form.pairs.length > 2) form.pairs.splice(idx, 1)
}
...
function goBack() {
  router.push({name: 'teacher-cabinet', query: {section: 'tests'}})
}
...
.type-btn.active i, .type-btn.active .tb-label {
  color: #fff;
}

.type-btn.active .tb-hint {
...
.type-btn.fill.active {
  border-color: var(--rust);
  background: var(--rust);
}

.type-btn.match.active {
...
.lv-btn.lv-easy.active {
  background: #2e7d32;
  border-color: #2e7d32;
  color: #fff;
}

.lv-btn.lv-mid.active {
  background: var(--gold);
  border-color: var(--gold);
  color: #fff;
}

.lv-btn.lv-hard.active {
  background: var(--rust);
  border-color: var(--rust);
  color: #fff;
}
...
.add-pair-btn {
  ...
  border: 1.5px dashed rgba(196, 146, 42, .4);
  border-radius: 2px;
  color: var(--gold);
  padding: .42rem .85rem;
  font-family: 'Source Serif 4', serif;
...
.preview-card.mcq::before {
  background: var(--gold);
}

.preview-card.truefalse::before {
  background: var(--sage);
}

.preview-card.fillblank::before {
  background: var(--rust);
}

.preview-card.match::before {
  background: var(--navy);
}
