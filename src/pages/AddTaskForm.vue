        ...
        <div class="form-card">
          ...
          <div class="fgrid">
            ...
            <div class="field">
              ...
              <div class="topic-wrap" :class="{ err: ve.topic }">

                <!-- Таңдалған немесе placeholder -->
                <button type="button" class="topic-trigger"
                        @click="topicOpen = !topicOpen">
                  <span :class="form.topic ? 'tp-selected' : 'tp-placeholder'">
                    {{ form.topic || '— Таңдаңыз —' }}
                  </span>
                  <i class="pi pi-chevron-down tp-arrow"
                     :class="{ rotated: topicOpen }"/>
                </button>

                <!-- Dropdown -->
                <div v-if="topicOpen" class="topic-dropdown">

                  <!-- Тізім -->
                  <div v-for="t in topicsList" :key="t.id" class="td-item"
                       :class="{ active: form.topic === t.name }"
                       @click="selectTopic(t.name)">
                    <span>{{ t.name }}</span>
                    <button type="button" class="td-del"
                            @click.stop="deleteTopic(t)"
                            title="Supabase-тен өшіру">
                      <i class="pi pi-times"/>
                    </button>
                  </div>
                  ...
                  <div class="td-add-row">
                    <input
                        ...
                        class="td-add-input"
                        placeholder="Жаңа тақырып…"
                        @keydown.enter.prevent="addNewTopic"
                        @click.stop
                    />
                    <button type="button" class="td-add-btn"
                            @click.stop="addNewTopic"
                            :disabled="!newTopicInput.trim() || topicSaving">
                      <div v-if="topicSaving" class="td-spin"/>
      ...
      <div class="preview-col">
        <div class="sticky-wrap">
          ...
          <div class="ls-info-card">
            <div class="li-head">
              <i class="pi pi-database" style="color:#3ecf8e"/> Supabase · questions
            </div>
            <div class="li-rows">
              <div class="li-row">
              ...
              <div class="li-row">
                <span><i class="pi pi-list-check"/> MCQ тест</span>
            ...
            <button class="li-reload" @click="init(true)" :disabled="loading">
...
const { tasks, loading, stats, init, addTask, updateTask, getById } = useTasksStore()

// ── Constants ─────────────────────────────────────────
const TYPES = [
  { val:'mcq',       key:'mcq',   label:'MCQ Тест',      icon:'pi-list-check',  hint:'4 нұсқа'     },
...
// ── Тақырыптар — Supabase topics кестесі ─────────────
...
async function addNewTopic() {
  ...
  try {
    ...
    topicsList.value = [...topicsList.value, data].sort((a,b) => a.name.localeCompare(b.name))
...
async function deleteTopic(topic) {
  if (!confirm(`«${topic.name}» тақырыбын өшіру?`)) return
  const { supabase } = await import('@/composables/useSupabase')
  await supabase.from('topics').delete().eq('id', topic.id)
  topicsList.value = topicsList.value.filter(t => t.id !== topic.id)
  if (form.topic === topic.name) form.topic = ''
}

function onClickOutside(e) {
  if (!e.target.closest('.topic-wrap')) topicOpen.value = false
}
...
onMounted(async () => {
  ...
  await Promise.all([ init(), loadTopics() ])
  if (editId.value) {
...
function loadTask(task) {
  ...
  form.level       = task.level || 'Оңай'
  form.text        = task.text  || ''
  form.explanation = task.explanation || ''
  form.answer      = task.answer ?? null
  form.answersRaw  = (task.answers || task.correctBlanks || []).join(', ')
...
function validate() {
  ...
  if (form.type === 'mcq') {
    ...
    if (mcqOptions.value.filter(o => o.trim()).length < 2) {
      ve.answer = 'Кем дегенде 2 нұсқа толтырыңыз'; ok = false
    }
  } else if (form.type === 'truefalse') {
    if (!form.text.trim())      { ve.text   = 'Тұжырым жазыңыз'; ok = false }
    ...
    if (form.pairs.filter(p => p.left.trim() && p.right.trim()).length < 2) {
      ve.pairs = 'Кем дегенде 2 толық жұп керек'; ok = false
    }
  }
  return ok
}
...
async function save() {
  ...
  try {
    ...
    if (editMode.value && editId.value) {
      ...
      showToast('Тапсырма Supabase-те жаңартылды!')
    } else {
      await addTask(data)
      showToast('Тапсырма Supabase-ке қосылды!')
      resetForm()
    }
  } catch(e) {
...
function buildTaskData() {
  ...
  if (form.type === 'mcq') return {
    ...base, text: form.text, answer: form.answer,
    optionA: mcqOptions.value[0], optionB: mcqOptions.value[1],
    optionC: mcqOptions.value[2], optionD: mcqOptions.value[3],
  }
  if (form.type === 'truefalse') return { ...base, text: form.text, answer: form.answer }
  ...
  if (form.type === 'match') {
    ...
    return { ...base, title: form.title, text: form.title, pairs }
  }
  return base
}
...
/* Form card */
.form-card { background:#fff; border:1px solid var(--border); border-radius:3px; overflow:visible; }
.form-card > .fs-head { border-radius:3px 3px 0 0; overflow:hidden; }
...
.topic-wrap.err .topic-trigger { border-color:var(--rust); }
...
.topic-trigger {
  ...
  text-align:left; transition:border-color .2s; color:var(--dark);
}
.topic-trigger:hover,
.topic-trigger:focus { border-color:var(--gold); outline:none; }
.tp-selected    { color:var(--dark); }
.tp-placeholder { color:#b0a090; font-style:italic; }
.tp-arrow  { font-size:.7rem; color:#b0a090; transition:transform .2s; flex-shrink:0; margin-left:.5rem; }
.tp-arrow.rotated { transform:rotate(180deg); }
...
.topic-dropdown::-webkit-scrollbar { width:4px; }
.topic-dropdown::-webkit-scrollbar-thumb { background:rgba(196,146,42,.25); border-radius:2px; }

.td-item {
  display:flex; align-items:center; justify-content:space-between;
  padding:.58rem .9rem; cursor:pointer; transition:background .15s;
  font-size:.85rem; color:var(--dark);
  border-bottom:1px solid rgba(217,205,184,.4);
}
.td-item:last-of-type { border-bottom:none; }
.td-item:hover  { background:#fdf5e6; }
.td-item.active { background:rgba(196,146,42,.1); color:var(--gold); font-weight:600; }

.td-del {
  opacity:0; background:none; border:none; cursor:pointer;
  color:#c0a890; font-size:.68rem; padding:.15rem .35rem;
  border-radius:2px; transition:all .15s; flex-shrink:0; line-height:1;
}
.td-item:hover .td-del { opacity:1; }
.td-del:hover { background:rgba(139,58,30,.12); color:var(--rust); }

.td-add-row {
  display:flex; gap:0;
  border-top:2px solid rgba(196,146,42,.2);
  padding:.5rem .6rem;
  background:#faf6ef;
...
.td-add-input {
  ...
  border-radius:2px 0 0 2px; padding:.4rem .65rem;
  font-family:'Source Serif 4',serif; font-size:.82rem;
  color:var(--dark); outline:none; background:#fff;
}
.td-add-input:focus { border-color:var(--gold); }
.td-add-input::placeholder { color:#b0a090; font-style:italic; }
.td-add-btn {
  background:var(--gold); border:1.5px solid var(--gold);
  border-left:none; border-radius:0 2px 2px 0;
  color:#fff; cursor:pointer; padding:.4rem .75rem;
  font-size:.82rem; transition:background .2s;
  display:flex; align-items:center; justify-content:center; min-width:34px;
}
.td-add-btn:hover:not(:disabled) { background:var(--gold-l); }
.td-add-btn:disabled { opacity:.4; cursor:not-allowed; }
.td-spin { width:12px; height:12px; border:2px solid rgba(255,255,255,.3); border-top-color:#fff; border-radius:50%; animation:spin .7s linear infinite; }
