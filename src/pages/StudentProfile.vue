      ...
      <div v-else class="sp-tab-content">
        ...
        <div v-if="activeTab === 'results'">
          ...
          <div class="results-cards">
            <div v-for="r in myResults" :key="r.id" class="result-card">
              <div class="rc-left">
                <div class="rc-pct-ring">
                  <svg viewBox="0 0 48 48">
                    <circle cx="24" cy="24" r="18" fill="none"
                            stroke="rgba(196,146,42,.15)" stroke-width="4"/>
                    <circle cx="24" cy="24" r="18" fill="none"
              ...
              <div class="rc-body">
                ...
                <div class="rc-meta">
                  <span>
                    <i class="pi pi-check-circle" style="color:#3a5c3a"/>
                    {{ r.correct }} / {{ r.total }} дұрыс
                  </span>
                  <span v-if="r.level">
                    <i class="pi pi-signal"/> {{ r.level }}
                  </span>
                  <span>
                    <i class="pi pi-calendar"/> {{ r.date }}
                  </span>
                </div>
                <span class="rc-grade" :class="gradeClass(r.grade)">
                  {{ r.grade }}
                </span>
              </div>
            </div>
            ...
            <div v-if="!myResults.length" class="sp-empty">
              <i class="pi pi-list"/>
              <p>Нәтиже жоқ — тест тапсырыңыз!</p>
              <p class="sp-empty-hint">
                <i class="pi pi-database"/> Нәтижелер Supabase-ке сақталады
              </p>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'profile'">
          <div class="profile-info-card">
            ...
            <p class="pic-note">
              ...
              Деректер <strong>Supabase</strong>-те сақталады.
            </p>
          </div>
        </div>
...
const {stats: commentsStats, init: commentsInit, getByStudent} = useCommentsStore()
...
// ── Init: Supabase-тен жүктеу ─────────────────────────
onMounted(async () => {
  try {
    // Пікірлер store-ын init ету (CommentsSection үшін де керек)
    await commentsInit()

    // Нәтижелер — Supabase results кестесінен
    const data = await fetchResults({
      studentId: currentUser.value?.id,
      limit: 200,
    })

    // Supabase field аттарын → app format-қа аудару
    myResults.value = (data || []).map(r => ({
      id: r.id,
      taskName: r.quiz_topic || '',
      topic: r.quiz_topic || '',
      level: r.quiz_level || '',
      correct: r.correct || 0,
      total: r.total || 0,
    ...
    console.error('[StudentProfile]', e)
  } finally {
    loading.value = false
  }
})
...
const avgScore = computed(() => {
  ...
  return Math.round(
      myResults.value.reduce((s, r) => s + r.pct, 0) / myResults.value.length
  )
})

const avgColor = computed(() =>
    avgScore.value >= 80 ? '#3a5c3a'
        : avgScore.value >= 50 ? '#c4922a'
            : '#8b3a1e'
)

// Оқушының өз пікір саны
const myCommentsCount = computed(() => {
  if (!currentUser.value?.id) return 0
  return getByStudent(currentUser.value.id).value?.length || 0
})

const headerStats = computed(() => [
  {val: myResults.value.length, label: 'Тапсырма'},
  {val: avgScore.value + '%', label: 'Орт. балл'},
  {val: myCommentsCount.value, label: 'Пікір'},
...
const gradeBreakdown = computed(() => {
  ...
  return defs.map(d => ({
    ...
    pct: Math.round(
        myResults.value.filter(r => r.grade === d.key).length / total * 100
    ),
  }))
})
