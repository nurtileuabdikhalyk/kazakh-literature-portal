          ...
          <div class="section-card">
            ...
            <div class="results-table-wrap">
              <table class="results-table">
                ...
                <tbody>
                <tr v-for="r in storeResults.slice(0,8)" :key="r.id || (r.studentId+r.taskTitle+r.date)">
                  <td class="rt-name">{{ r.studentName }}</td>
                  <td>{{ r.class }}</td>
                  <td class="rt-task">{{ r.taskTitle || r.taskName }}</td>
                  <td>
          ...
          <div class="section-card">
            <div class="sc-head">
              ...
              <div class="sc-head-actions">
                <span class="sc-source-badge">
                  <i class="pi pi-database"/> results_store_v1
                </span>
            ...
            <div v-else class="results-table-wrap">
              <table class="results-table">
                ...
                <tbody>
                <tr v-for="r in filteredResults" :key="r.id || (r.studentId+r.date)">
                  <td class="rt-name">{{ r.studentName }}</td>
...
const {
  ...
  stats: resultsStats,
  init: resultsInit,
  deleteResult: resultsDelete,
  exportToExcel: resultsExport,
} = resultsStore

const resultsLoading = ref(false) // UI-only loading state
...
const navItems = computed(() => [
  ...
  {key: 'results', label: 'Нәтижелер', icon: 'pi-chart-bar', badge: storeResults.value.length || null},
...
onMounted(async () => {
  try {
    ...
    tasksStore.init()
    commentsStore.init()
    resultsInit()   // sync — localStorage-тен жылдам оқиды
...
const filteredResults = computed(() => {
  ...
  if (resultsSearch.value.trim()) {
    ...
    list = list.filter(r =>
        ...
        (r.taskTitle || r.taskName || '').toLowerCase().includes(q)
    )
  }
...
function doDeleteResult() {
  if (deleteResultDialog.result?.id) resultsDelete(deleteResultDialog.result.id)
...
.lesson-card-fade-enter-active, .lesson-card-fade-leave-active {
  transition: opacity .25s, transform .25s;
}

.lesson-card-fade-enter-from, .lesson-card-fade-leave-to {
  opacity: 0;
  transform: scale(.96);
}
